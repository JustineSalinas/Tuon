    ...
    <main className="mx-auto w-full max-w-5xl px-4 py-6 md:px-8 md:py-10">
      <header>
        ...
        <p className="text-muted-foreground mt-1.5 text-sm">
          {linkedCount > 0
            ? t.graph.summary(linkedCount, graph.edges.length)
            : t.graph.subtitle}
        </p>
      </header>
      ...
      {loading ? (
        ...
        <EmptyGraph hasNotes={notes.length > 0} t={t} />
      ) : (
...
function EmptyGraph({ hasNotes, t }: { hasNotes: boolean; t: Messages }) {
  return (
    <div className="mt-10 rounded-2xl border border-dashed py-16 text-center">
      ...
      <Button className="mt-6" render={<Link href={hasNotes ? "/app/notes" : "/app/notes/new"} />}>
        <Link2 />
        {hasNotes ? t.graph.openNotes : t.graph.writeFirst}
      </Button>
    </div>
  );
}
